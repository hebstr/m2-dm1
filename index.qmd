...
format:
  hebstr-html:
    code-tools:
      source: https://github.com/hebstr/m2-dm1
---
...
#| echo: true
#| code-fold: false
glimpse(read_csv2("data/retinol.csv"))
```
...
.title_ql <- str_fig(
  title = "Caractéristiques des variables qualitatives.",
...
#|   group: fig-qt
.title_qq <- str_fig(
  title = "Diagrammes Q-Q.",
  note = str_glue(
    "Distribution {str_color('théorique', .pal[1])} en abscisses,
...
.title_dty <- str_fig(
  title = str_glue(
    "Densité de fréquence des variables quantitatives chez les
    {str_color('hommes', .pal[1])} et les {str_color('femmes', .pal[2])}."
  ),
  note = str_glue(
    "Les moyennes sont représentées par les traits en pointillés,
...
```{r}
#| echo: true
.calc_p <- \(x, y) cor.test(df[[x]], df[[y]])$p.value
...
df |>
  ...
  mutate(
    r = round(r, 2),
    p.value = map2_dbl(x, y, .calc_p) |> style_pvalue()
  ) |>
  arrange(desc(r)) |>
  head(n = 3) |>
  gt_qmd() |>
...
#| code-fold: false
.chisq <- lst(
  vars = combn(opts$data$ql$vars, 2),
  expected = map2(
    .x = vars[1, ] |> set_names(str_glue("{vars[1, ]}-{vars[2, ]}")),
    .y = vars[2, ],
...
.title_bv <- list(
  ...
  map(~ str_glue("Caractéristiques des variables en fonction {.}"))
```

{{< script scripts/tbl_bv.R >}}
